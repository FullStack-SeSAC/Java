...
public class ConditionalStatement {
    ...
    public static void main(String[] args) {
        ...
        /*
        WHY?
        - `==` 연산자 : 두 객체의 "참조"를 비교(동일한 메모리 위치를 가르키는지 검사) => "객체(참조) 동등성"
        - `.equals()` 메서드 : 두 객체의 "내용"이 동일한지 비교 => "내용 동등성"
         */

        /*
        "문자열 리터럴"의 경우, Java에서 특별한 취급. 동일한 문자열 리터럴이 사용된 경우,
        ...
        /*
        "문자열 동적 할당"의 경우, `new String(...)`를 사용해 새로운 문자열 객체를 생성하면 서로 다른 객체를 가르킴
        즉, str3와 str4가 다른 객체이므로 `==` 연산자로 비교시 false.
        => Java에서 문자열 내용을 비교하고 싶을 떼, `.equals()`매소드로 비교 지향
         */
        ...
        /*
        if ~ else if 문
        top - down 방식으로 청므 조건식이 참으로 평가 시, 해당 식 수행한 다음에 if 절을 빠져나감
         */
        if (number % 3 == 0) {
        ...
        /*
        switch ~ case 문
        - 각 case 문의 break 문은 선택사항으로 break 문 생략시 바로 밑의 case 문으로 연결 (여러 case 문으로 하나의 시행문 지정 가능)
         */
        String dayOfWeek;
        int day = 14;
