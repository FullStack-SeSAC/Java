...
// - non-static method: 객체에 속하는 매서드, 객체 생성 후 호출 가능
...
public class Method {
    ...
    // 가변인자(Varargs) 사용: 메소드 호출 시 넘겨줄 인자의 개수를 동적으로 지정하는 기능
    // 문법) 타입... 변수명
    // - 매소드 내부에서 가변 인자는 배열로 처리
    // - 가변 인자는 메소드의 마지막 파라미터로만 사용 가능
...
// 용어 정리
// - 메소드 선언 : 이름 있는 중괄호 블럭 만들기
// - 메소드 호출 : 이름으로 중괄호 블럭 실행하기